  ...
  const getTemperatureColor = (temp: number): string => {
    if (temp < 0) return 'rgba(59, 130, 246, 0.15)'; // синий для морозов
    if (temp < 10) return 'rgba(147, 197, 253, 0.15)'; // светло-синий для холода
    if (temp < 20) return 'rgba(251, 146, 60, 0.15)'; // прозрачно-оранжевый 10-20°C
    if (temp < 30) return 'rgba(239, 68, 68, 0.15)'; // красно-прозрачный 20-30°C
  ...
  const getAirQualityColor = (aqi: number): string => {
    if (aqi <= 50) return 'rgba(34, 197, 94, 0.15)'; // зеленый
    if (aqi <= 100) return 'rgba(251, 191, 36, 0.15)'; // желтый
    if (aqi <= 150) return 'rgba(251, 146, 60, 0.15)'; // оранжевый
    if (aqi <= 200) return 'rgba(239, 68, 68, 0.15)'; // красный
    if (aqi <= 300) return 'rgba(147, 51, 234, 0.15)'; // фиолетовый
    ...
    useEffect(() => {
      ...
      load().then(({ Map, CircleMarker }) => {
        ...
        const createCircleOverlay = () => {
          ...
          let color = 'rgba(100, 100, 100, 0.1)';
          ...
          circleRef.current = new CircleMarker(mapRef.current, {
            ...
            radius: radius,
            color: color,
            strokeColor: color.replace('0.15', '0.3').replace('0.4', '0.7').replace('0.5', '0.8'),
            strokeWidth: 2,
          });
        };
